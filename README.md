...
- **Step 2**: Features are then extracted from the images in `data/img/train_<filenb>.h5` files using a **pre trained ResNet50 model** and stored in a new .h5 file `data/features/resnet_train_<filenb>.h5`. 
...
Each caption goes through a simple pre-processing 'clean up' before its final storage. It is lower cased, and all numerical characters, punctuation, and hanging ’s’ and ‘a’ are removed. 
